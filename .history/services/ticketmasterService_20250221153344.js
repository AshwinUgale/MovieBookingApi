...
const fetchVenues = async (city = "New York") => {
    try {
        console.log(`📌 Fetching venues from Ticketmaster for city: ${city}`);
        console.log("🔑 Using API Key:", config.TICKETMASTER_API_KEY);

        // Fetch venues ONLY (ensuring we're not pulling events)
        const response = await axios.get(`${TICKETMASTER_BASE_URL}/venues.json`, {
            params: {
                apikey: config.TICKETMASTER_API_KEY,
                size: 50, // Fetch more venues for better filtering
            },
        });

        const allVenues = response.data._embedded?.venues || [];

        // 🔍 Manually filter to match ONLY venues in the requested city
        const filteredVenues = allVenues.filter(venue => 
            venue.city?.name?.toLowerCase() === city.toLowerCase()
        );

        console.log(`📌 Found ${filteredVenues.length} venues in ${city}`);
